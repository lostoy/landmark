...
from init_pretrained_model import prepare_model
...
def prepare_stats(context):
    if context['args'].step != -1:
        context['step'] = context['args'].step
...
def main():
    ...
    if args.evaluate:
        print('start testing...')
        validate(context, force_validate=True, is_test=True, force_save=False)
        return
...
def validate(context, force_validate=False, is_test=False, force_save=False):
    ...
    while True:
        ...
        if step == 0 and not force_validate:
            break
        timer.tic()
